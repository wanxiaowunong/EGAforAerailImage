...
class VaihingenDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(512, 512), mean=(128, 128, 128), scale=False,
                 mirror=True, ignore_label=255, set='train'):
        self.root = root
        ...
        for name in self.img_ids:
            img_file = osp.join(self.root, "image/%s%s" % ( name,".tif"))
            lbl_file = osp.join(self.root, "label/%s%s" % (name, ".tif"))
            self.files.append({
                "img": img_file,
                "label": lbl_file,
                "name": name
...
if __name__ == '__main__':
    dst = VaihingenDataSet("./data", is_transform=True)
