...
SNAPSHOT_DIR = './snapshots_resnet_plot_d_all/'
...
DATA_DIRECTORY_val = '/Share/home/data/ISPRS512_512/Vaihingen'
DATA_LIST_PATH_val = '/Share/home/data/ISPRS512_512/Vaihingen/16.txt'
...
def main():
    ...
    for i_iter in range(args.num_steps):
        ...
        loss_dcl = bce_loss(prob_dcl_real,
                            Variable(torch.FloatTensor(prob_dcl_real.data.size()).fill_(source_label)).cuda()).cuda() \
                   + bce_loss(prob_dcl_fake,
                              Variable(torch.FloatTensor(prob_dcl_fake.data.size()).fill_(target_label)).cuda()).cuda()
        loss_dcl.backward()
        ...
        if (i_iter + 1) % 936 == 0:
            ...
            plt.title('')
            loss_dcl_tmp.append(loss_dcl_tmp0 / 936)
            loss_dcl_tmp0 = 0
            plt.plot(epoch_tmp, loss_dcl_tmp, label='loss_dcl')
            plt.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3, borderaxespad=0.)
            plt.grid()
