...
SNAPSHOT_DIR = './snapshots_gcn2_entropy_PM/'
...
def main():
    ...
    for i_iter in range(args.num_steps):
        ...
        '''weighted global adversarial loss'''
        D_out = interp_target(model_D(F.softmax(pred_target, dim=1)))
        if (i_iter > PREHEAT_STEPS):
            loss_adv = weighted_bce_loss(D_out,Variable(torch.FloatTensor(D_out.data.size()).fill_(source_label)).cuda(),
