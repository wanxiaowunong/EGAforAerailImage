...
'''使用基于GCN的LSR模块增强上下文推理能力'''
#IMG_MEAN = np.array((104.00698793, 116.66876762, 122.67891434), dtype=np.float32)
IMG_MEAN_P = np.array((85.92, 92.55, 86.55), dtype=np.float32)  #Potsdam数据集中每个通道的均值
IMG_MEAN_V = np.array((81.19, 81.80, 120.48), dtype=np.float32) #Vaihingen数据集中每个通道的均值
...
if SOURCE == 'Potsdam':
    ...
    Lambda_local = 40
    Epsilon = 0.4
    DATA_DIRECTORY_TARGET ='/Share/home/data/ISPRS512_512/Vaihingen'
    DATA_LIST_PATH_TARGET = '/Share/home/data/ISPRS512_512/Vaihingen/17.txt'
    ...
    Lambda_local = 30
    Epsilon = 0.4
    DATA_DIRECTORY_TARGET ='/Share/home/data/ISPRS512_512/Potsdam_nooverlap'
    DATA_LIST_PATH_TARGET = '/Share/home/data/ISPRS512_512/Potsdam_nooverlap/14_nooverlap.txt'
...
def main():
    ...
    for i_iter in range(args.num_steps):
        ...
        if i_iter >= args.num_steps_stop - 1:
            print('save model ...')
            if args.source == 'Potsdam':
                torch.save(gcn_model.state_dict(), osp.join(args.snapshot_dir, 'P2V_gcn' + str(args.num_steps) + '.pth'))
            else:
                torch.save(gcn_model.state_dict(), osp.join(args.snapshot_dir, 'V2P_gcn' + str(args.num_steps) + '.pth'))
