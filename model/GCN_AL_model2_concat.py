...
'''利用图卷积网络的消息传播机制聚合上下文特征，增强特征表示能力,concat'''
...
class GCN(nn.Module):
    def __init__(self, nin, nhid, nout, dropout):
        ...
        self.fc_2 = nn.Conv2d(512, 1024, kernel_size=1, padding=0, stride=(1, 1),
                              groups=1, bias=False)
        self.blocker = nn.BatchNorm2d(1024)

    def forward(self, x, adj):
        x = F.relu(self.gc1(x, adj))
        x = F.dropout(x, self.dropout, training=self.training)
        x = self.gc2(x, adj)  #512
...
class GCN_Resnet(nn.Module):
    def __init__(self, num_classes):
        ...
        self.conv53 = nn.Sequential(nn.Conv2d(2048+1024, 2048, 1, padding=0, bias=False),  # 512,512
                                    nn.BatchNorm2d(2048),
                                    nn.ReLU())
    ...
    def forward(self, x):
        ...
        feature_map1=self.conv51(feature_map)  # n*1024*h*w

        gcn_in=self.conv52(feature_map1) #n*512*h*w # used to constract graph

        '''constract graph structure'''
        ...
        # gcn_out = gcn_out.reshape([gcn_out.shape[0], int(gcn_out.shape[1] ** 0.5), int(gcn_out.shape[1] ** 0.5)])
        # gcn_out = gcn_out.unsqueeze(0)
        concat_f = torch.cat([feature_map, gcn_out], dim=1) #2048+1024
        # print(gcn_out.shape)
